...
`Navbar.tsx`'s six tabs are a **3-column grid on mobile** (`grid grid-cols-3 sm:flex`) that falls back to the original single row at `sm`. As a plain flex row they don't fit a phone: the links shrink and each label wraps *inside itself* (프로젝 / 트), so the links also carry `whitespace-nowrap` and `px-2 sm:px-3` — the tighter padding is what keeps 체크리스트 intact at 320px. The 로그아웃 block is `ml-auto sm:order-last` so it shares row one with the wordmark on mobile without moving on desktop.
