...
- `schedule_items` — per-user, per-date (not per-project) — a flat daily to-do list shown on `/dashboard`, unrelated to `project_checklist_items`. `/calendar` renders these **plus** any `work_logs` row with `status = 'todo'` and a `next_action_date` in range, merged into one `CalendarEntry` union per day cell. Log-derived entries are tinted per project by `projectColorClass()` (`src/lib/projectColor.ts`), which hashes the project id into a fixed 10-colour palette — the colour is derived, not stored, so the same site is always the same colour without a migration. Those Tailwind classes are written out in full because Tailwind cannot see dynamically built class names; `src/lib/**` had to be added to `tailwind.config.ts`'s `content` for them to survive purging. Anything that changes a log's status/next_action must `revalidatePath("/calendar")` — all four actions in `dashboard/actions.ts` do.
